    ...
    $.get(BASE + 'listar', { estado: filtroEstado }, function (res) {
        ...
        if (!datos.length) {
            html = '<div class="tabla-vacia">' +
                '<i class="bi bi-arrow-left-right ani-mover"></i>' +
            ...
            datos.forEach(function (p) {
                ...
                html += '<div class="pase-card" onclick="abrirDetalle(' + p.id + ')">' +
                    '<div class="pase-flecha"><i class="bi bi-arrow-left-right ani-mover"></i></div>' +
                    '<div class="pase-info">' +
                        ...
                        '</div>' +
                        '<div class="pase-meta">' +
                            '<i class="bi bi-arrow-right ani-mover"></i> De <b>' + paseEscHtml(p.de_turno) + '</b> a <b>' + paseEscHtml(p.a_turno) + '</b>' +
                            ' &middot; ' + paseFormatoFecha(p.fecha) +
                            ' &middot; <i class="bi bi-circle"></i> ' + paseEscHtml(p.creador_nombre) +
...
function recargarDetalle() {
    ...
    $.when(
        ...
        $('#detTitulo').html('<i class="bi bi-circle ani-mover"></i> De ' +
        ...
        $('#detMeta').html(
            ...
            ' &middot; Creado por <i class="bi bi-circle"></i> ' + paseEscHtml(pase.creador_nombre) +
        ...
        claves.forEach(function (area) {
            ...
            html += '<div class="area-grupo">' +
                '<div class="area-grupo-head">' +
                    '<span><i class="bi bi-grid ani-latido"></i> ' + paseEscHtml(area) + '</span>' +
...
function renderPunto(pp, estadoPase) {
    ...
    var acciones = '';
    if (esAdmin) {
        acciones += '<button class="btn-sm-pase" onclick="editarPunto(' + pp.id + ')"><i class="bi bi-pencil ani-sacudida"></i> Editar</button>';
    }
    acciones += '<button class="btn-sm-pase cyan" onclick="cambiarEstadoPunto(' + pp.id + ', \'revisado\')"><i class="bi bi-eye ani-latido"></i> Revisado</button>';
    acciones += '<button class="btn-sm-pase green" onclick="cambiarEstadoPunto(' + pp.id + ', \'completado\')"><i class="bi bi-check-lg ani-latido"></i> Completado</button>';
    acciones += '<button class="btn-sm-pase amber" onclick="cambiarEstadoPunto(' + pp.id + ', \'pendiente\')"><i class="bi bi-arrow-counterclockwise ani-girar"></i> Pendiente</button>';
    if (esAdmin && !pp.tarea_id) {
        acciones += '<button class="btn-sm-pase blue" onclick="abrirModalTarea(' + pp.id + ')"><i class="bi bi-list-task ani-mover"></i> Convertir en tarea</button>';
    }
    if (pp.tarea_id) {
        acciones += '<a class="vinculo-tarea" href="' + BASE_URL + 'tareas" target="_blank"><i class="bi bi-list-task ani-mover"></i> Tarea vinculada</a>';
        if (esAdmin) {
            acciones += '<button class="btn-sm-pase red" onclick="desvincularTarea(' + pp.id + ')"><i class="bi bi-unlink ani-sacudida"></i></button>';
        }
    }
    acciones += '<button class="btn-sm-pase" onclick="toggleComentarios(' + pp.id + ', this)"><i class="bi bi-chat-dots ani-latido"></i> Comentarios</button>';
    if (esAdmin || pp.creado_por == USUARIO_ID) {
        acciones += '<button class="btn-sm-pase red" onclick="eliminarPunto(' + pp.id + ')"><i class="bi bi-trash ani-sacudida"></i></button>';
    }
    ...
    return '<div class="punto-item ' + pp.estado + '">' +
        ...
        '<div class="punto-meta">' +
            ...
            '<span class="autor"><i class="bi bi-circle"></i> ' + paseEscHtml(pp.creador_nombre) + '</span>' +
            '<span>' + paseFormatoFecha(pp.created_at) + '</span>' +
            (pp.actualizado_por && pp.actualizado_nombre && pp.created_at !== pp.updated_at
                ? '<span class="autor"><i class="bi bi-pencil ani-sacudida"></i> Editado por ' + paseEscHtml(pp.actualizado_nombre) + ' ' + paseFormatoFecha(pp.updated_at) + '</span>'
                : '') +
        '</div>' +
...
function abrirModalPunto(id) {
    ...
    $('#puntoContenido').val('');
    if (id) {
        $('#puntoModalTitulo').html('<i class="bi bi-pencil ani-sacudida"></i> Editar punto');
    } else {
        $('#puntoModalTitulo').html('<i class="bi bi-pin ani-latido"></i> Nuevo punto');
    }
...
function cargarComentarios(puntoId) {
    $.get(BASE + 'comentarios/' + puntoId, function (res) {
        ...
        if (comentarios.length) {
            comentarios.forEach(function (c) {
                html += '<div class="comentario-item">' +
                    '<span class="autor-c"><i class="bi bi-circle"></i> ' + paseEscHtml(c.autor_nombre) + '</span>' +
        ...
        html += '<div class="comentario-input">' +
            '<button type="button" class="com-adjuntar-btn" onclick="comAbrirSelectorPase(' + puntoId + ')" title="Adjuntar archivo"><i class="bi bi-paperclip ani-sacudir"></i></button>' +
            ...
            '<button class="btn btn-primary-custom btn-sm" onclick="guardarComentario(' + puntoId + ')"><i class="bi bi-send ani-mover"></i></button>' +
            '</div>' +
    ...
    $.get(BASE + 'turnos', function (res) {
        ...
        turnos.forEach(function (t) {
            html += '<div class="d-flex align-items-center gap-2 py-2" style="border-bottom:1px solid var(--border);">' +
                ...
                '</div>' +
                '<button class="btn-sm-pase blue" onclick="editarTurno(' + t.id + ')"><i class="bi bi-pencil ani-sacudida"></i></button>' +
                '<button class="btn-sm-pase red" onclick="eliminarTurno(' + t.id + ')"><i class="bi bi-trash ani-sacudida"></i></button>' +
            '</div>';
        });
